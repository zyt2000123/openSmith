...
## 落地状态（截至 2026-08-30，`b3253d9..` 共 20 个提交，6 轮修复-review 循环已收敛）

**20 条全部落地**，另加六轮对抗性 review 逐轮发现的补丁自身缺陷（前五轮每轮
都抓到上一轮修复拆掉的不写之约；第 6 轮穷举故障注入零违例，循环就此收敛）。
...
### 第 4–6 轮：/token 回填链路（server 侧，#15 的连带整改）

- **第 4 轮**：`_SUPERSEDED_*`/`_UNPRICED_*` 同轮判据从相关子查询下标式改为
  区间式（三方对拍 2200 轮 0 失配），EXPLAIN QUERY PLAN 护栏测试锁定
  `idx_token_usage_session_time` 的 SEARCH 路径。
- **第 5 轮**：回填移到 `dedicated_connection()` 私有连接，修掉共享连接上
  `rollback()` 吞掉别的协程已执行未提交写入的静默丢数据（实测 `add_message`
  的消息无声消失）。但随发的"500 行一提交"缓解在真实消息尺寸下不成立——
  它把静默丢失换成了用户可见的发送失败（`database is locked` → 该轮消息
  未落库）。
- **第 6 轮**（`38009d0` + `87c6a26`）：tiktoken 移出写事务（一次
  `to_thread` 预定价）；提交边界改为持锁时钟（50ms，500 行保留为 WAL/崩溃
  丢失量的第二道上限，实测无争用时行数闸先响）；trace 导入与两条全表
  DELETE 同样切批（游标 upsert 恒在其事件之后——反向才丢数据）；
  `OperationalError` 区分 `no such table` 与其它（两处）。持锁 max ≈20 倍
  降幅，并发写者失败 2/16→0/159。真并发回归测试锁住"回填不得挤掉用户
  消息"（修复前红 3/3，变异验证判别力；断言全为语义事实，无墙钟死线）。
  第 6 轮 review 对每个语句边界穷举注入（92+53 点）零违例、总量单调性
  85 采样零违例、`_estimate_token_counts` 与旧内联行为等价（全 Unicode
  枚举证伪了唯一疑似漂移），**并顺手堵了一个既有泄漏**：`sessions` 缺失
  的库上 DELETE 后 SELECT 抛错会留下开着的写事务，写锁扣到进程结束。

### 记账不阻塞（已知残留，另行立项，第 6 轮 review 量化过）

- **回填 INSERT 阶段仍可挤掉第二个写进程**：等待者赢不进 COMMIT 与下一条
  INSERT 之间的亚毫秒缝（busy handler 100ms 轮询），实际排队等整个 INSERT
  阶段（≈行数×55µs，与消息大小无关）。触发门槛已从 ~2 万行抬到 10–20 万
  行 + 第二个写进程（1/676）。根治 = 分块 SELECT（顺带解决下一条），会动
  第 4 轮 EQP 锁定的 SELECT 形状，需重跑护栏。
- **估算回填全量物化未定价消息正文**（60MB 内容 → RSS +22MB，线性）；本次
  未恶化（`token_counts` 仅多 int 列表）。
- **`record_usage` 在回填 SELECT 与 INSERT 之间定价的竞态**：窗口因 SELECT
  移出写事务而变宽，但构造不出跨重启存活的双计；后果止于下次启动前多一轮
  `estimated: true`（诚实标注）。
- **游标表 DDL 双份定义**（`schema.py:105` / `token_stats_service.py`）：
  今天字面一致，改一处会在空库上静默分叉；两处已互加注释。生产路径实测
  非缺陷（表已存在时 IF-NOT-EXISTS 不取写锁）。
- **新并发测试的断言 3 依赖 fixture 编码:插入 ≈6:1**：60k×80B 形态会在
  正确代码上翻红（其实是在诚实暴露上一条残留）。已在测试内注明"改 fixture
  必须重测两阶段，不许放宽 2x"。
