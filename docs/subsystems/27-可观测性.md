...
复用 `common/hash_chain.py`（见 [06 · 安全与安全边界](23-工具与安全.md) §10）。**run 结束时封存锚点**，所以"把 trace 回滚到一个更短但自洽的版本"也能被发现。
...
这和 [08 · Agents 内容层](../layers/42-Agents.md) §3.5 的 `\b` 是同一类问题：**边界断言不是装饰，去掉它正则就开始吃掉正常内容**。
...
同一个 `.jsonl` 文件里混着两类记录：执行事件和 prompt manifest（记录本次 prompt 由哪些层拼成，见 [04 · Engine 核心执行](20-Agent-Loop.md)）。manifest 的 `type` 不是合法的 `EventType`，构造时抛 `ValueError`，被接住跳过。
...
这六条里，②④⑥在其他子系统同样成立（见 [05 · 记忆系统](21-记忆系统.md) 的三道守卫、[06 · 安全与安全边界](23-工具与安全.md) 的守卫先于挑战），可以当作阅读这套代码的通用预期。
...
| 想深入 | 读 |
|---|---|
| 27 种事件类型分别是什么 | [03 · 架构总览](../architecture/10-系统架构.md) §3 |
| 哈希链的能力边界 | [06 · 安全与安全边界](23-工具与安全.md) §10 |
| `GenerationRecord` 怎么产生 | [07 · LLM 集成](25-LLM集成.md) §8 |
| 7 个可观测端点的签名 | [09 · Server API 层](../layers/43-Server.md) §2.5 |
| 崩溃恢复的完整链路 | [03 · 架构总览](../architecture/10-系统架构.md) §9.2 |
