...
见 [02 · 快速上手](../guide/02-快速上手.md) §3.1 的完整图。这里补三个实现细节。
...
这是 [02 · 快速上手](../guide/02-快速上手.md) §4.1 讲过的设计，这里补一下它解决的具体故障：
...
这正是 [13 · Common 基础设施](../layers/40-Common.md) §5.5 那条纪律的一个消费方实例——不能在模块级 `from common.config import PATHS`，否则 `reset_paths()` 对这个模块无效。对应的测试 `llm_config_resolves_paths_when_it_runs` 就守在这里。

### 2.3 三条路由与超时档

见 [02 · 快速上手](../guide/02-快速上手.md) §4.2/§4.4。补一条：`LLMTimeouts` 把五个字段翻译成 **两个** `httpx.Timeout`：
...
`LLMContextLengthError` 被单独提出来，是因为 ReAct 循环要**专门处理**它——`_is_context_limit_error()` 命中后会触发一次上下文恢复重试（见 [04 · Engine 核心执行](20-Agent-Loop.md) §4.1），而不是直接失败。
...
这套上限和 MCP 的四道边界（见 [12 · MCP 集成](26-MCP集成.md) §4）**是同一个设计模式的两次应用**：一个外部服务的响应流必须同时被字节、条数和时间三个维度约束，因为任何单一维度都有绕过方式。
...
所以要**按文本标记识别**，然后抛 `LLMContextLengthError`——ReAct 循环的 `_is_context_limit_error()` 靠这个类型决定要不要触发一次上下文恢复重试（见 [04 · Engine 核心执行](20-Agent-Loop.md) §4.1）。
...
Anthropic 的 prompt 缓存需要在消息里插一个显式的 `cache_control` 标记。**断点位置就是 `prefix_cache_key` 对应的那个稳定前缀边界**（见 [04 · Engine 核心执行](20-Agent-Loop.md) §2.4）。
...
这是 [04 · Engine 核心执行](20-Agent-Loop.md) §4.1 里那段 `getattr(getattr(llm, "capabilities", None), "prefix_cache_key", False)` 的另一半——调用方先问，客户端再验，**两层都不信任对方**。
...
这是"读-改-写"的经典竞态：两个轮次同时读到同样的旧内容，各自加上自己的一行再写回，**后写的覆盖先写的**，丢一行。触发场景很具体——主循环正在录制时，上下文压缩也发起了一次 LLM 调用（见 [09 · Server API 层](../layers/43-Server.md) §4.6）。
...
**① 结构化字段可信，自由文本不可信。** 重试分类只看 `type`/`code`/`status`（§15.1 ⑤），错误消息一律不进异常（§15.1 ④）、不参与判断。理由有两个：文本会变（provider 改文案就失效），文本会带 prompt（泄漏）。这条在 [10 · 可观测性与诊断](27-可观测性.md) §5.2.3 区分工具超时与审批超时时也成立。
...
举个反例：某个 provider 的流式响应需要额外的请求头才能带上用量数据。这看起来像个配置项，其实不是——用户没有理由选择"不要用量数据"，这是适配器为了满足既有契约必须做的事，应该硬编码在适配器里。而"用哪个模型"、"超时多久"、"要不要开思考模式"才是真正的选择，因为不同场景下用户会给出不同答案。这条判据同样适用于新增路由或超时档：如果所有人都会填同一个值，那它就是一个常量而不是配置项。
...
"不估算"这条尤其重要——一个估算出来的 token 数会混进成本统计，而使用者无法分辨哪些是真实上报、哪些是猜的。宁可是零，也不要一个看起来合理的假数字。这和 [10 · 可观测性与诊断](27-可观测性.md) §8.5 里 `None` 与 `0.0` 的区分是同一种考虑。
...
| 想深入 | 读 |
|---|---|
| 配置怎么从 YAML 到请求 | [02 · 快速上手](../guide/02-快速上手.md) §3–§4 |
| 事件怎么变成 ReAct 循环的行为 | [04 · Engine 核心执行](20-Agent-Loop.md) §4 |
| `GenerationRecord` 怎么落库 | [10 · 可观测性与诊断](27-可观测性.md) |
| 门禁与记忆编译各走哪条路由 | [05 · 记忆系统](21-记忆系统.md) §9.4 |
| `/api/config/llm` 端点 | [09 · Server API 层](../layers/43-Server.md) |
