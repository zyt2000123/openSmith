...
命令是**固定的 20 个**，技能不进这个面板（[11](../layers/44-Shell.md) §11 解释了为什么分开）。按你什么时候会用到它们分组：
...
**`/compact` 与 `/compress`。** 两个都压缩，但对象不同——一个针对当前会话的显示，一个触发服务端的上下文摘要（[09](../layers/43-Server.md) §4.6 讲了后者的实现，它会真的调一次 LLM 并把结果存进会话）。

**`/reload` 与 `/reconnect`。** `/reload` 重读上下文文件（`SMITH.md`）并开一个新会话，用于"我改了指令文件，想让它生效"；`/reconnect` 是重连后端进程，用于"后端挂了"。

**`/resume`。** 续跑一个未完成的 run。它受七道校验约束（[09](../layers/43-Server.md) §4.7），最常见的拒绝原因是"会话已经有更新的用户轮次"——也就是你在那个 run 崩了之后又说了新的话。
...
分成两个是因为数据量差着几个量级——列出最近 50 次运行不该去读 50 个 trace 文件（[10](../subsystems/27-可观测性.md) §2）。

`/trace` 展示的内容已经过两层脱敏（[10](../subsystems/27-可观测性.md) §3.2），所以里面看不到 API key，但**能看到工具的完整参数**——这正是排查"它为什么读了那个文件"的关键信息。
...
| | `AGENT_SMITH_PROJECT_ROOT` | 用户，仅在自动发现失败时（[13](../layers/40-Common.md) §2.7） |
| | `AGENT_SMITH_OBSERVABILITY_MAX_RUNS` / `_MAX_AGE_DAYS` / `_MAX_BYTES` | 运维，调保留策略（[10](../subsystems/27-可观测性.md) §9.5） |
| **调试类** | `AGENT_SMITH_RECORD_LLM` | 开发者，录制回放（[07](../subsystems/25-LLM集成.md) §12） |
| | `AGENT_SMITH_FACT_GATE` | 开发者，临时关掉事实门（[06](../subsystems/23-工具与安全.md) §7.4） |
| **进程内部** | `SMITH_SERVER_NONCE` / `SMITH_SERVER_URL` / `SMITH_PROJECT_CWD` / `SMITH_REPO_ROOT` | **shell 自己设**，用户不该动（[11](../layers/44-Shell.md) §10.3） |
...
| 现象 | 最可能的原因 | 怎么确认 |
|---|---|---|
| 启动即报 "must point to an Agent-Smith root" | `AGENT_SMITH_PROJECT_ROOT` 指错了 | 检查那个目录下有没有 `agents/smith/config.yaml`、`agents/identities/smith.yaml`、`agents/skills/*/SKILL.md` 三样（[13](../layers/40-Common.md) §2.7） |
| 启动报 "Unable to locate an Agent-Smith project root" | 自动发现失败 | 从当前目录往上没有合格的根；显式设 `AGENT_SMITH_PROJECT_ROOT` |
| LLM 请求报配置错误但 key 明明填了 | `base_url` 没通过校验 | 必须是 https、不能带 query/fragment/userinfo、不能指向私有地址（[07](../subsystems/25-LLM集成.md) §2.2.1） |
| 模型列表能拉到但调用失败 | 中转站列了没开通道的模型 | `curl` 直接打一次那个模型（[07](../subsystems/25-LLM集成.md) §15） |
| MCP 工具完全没出现，日志也没报错 | `command` 写成了字符串而不是列表 | YAML 里 `command: npx foo` 是字符串，会被**静默跳过**（[12](../subsystems/26-MCP集成.md) §9.3） |
| 改了代码但行为没变 | 后端进程没重启 | 查 `/api/health` 的 `stale` 字段（[09](../layers/43-Server.md) §10.1） |
| 健康检查通过但所有请求 404 | 部署在子路径反向代理后面 | 两处 URL 拼接方式不同（[11](../layers/44-Shell.md) §5.8.2） |
| 技能装了但用不了 | 目录里没有顶层 `SKILL.md` | `find agents/skills -maxdepth 2 -name SKILL.md`（[08](../layers/42-Agents.md) §1.1） |
| Agent 读不了 `.env.example` | 不应该——这是模板，明确放行 | 若真被拦是 bug（[08](../layers/42-Agents.md) §10.1.2） |
| 每次工具调用都弹审批 | 用的是 MCP 工具或 `shell` | 这两类一律 `approval_policy=always`（[08](../layers/42-Agents.md) §4.1） |
...
自动任务用 `lease_until` + `lease_token` 两列做互斥（见 [01 · 总览](01-产品定位.md) §7.3）。崩溃重启时只重置**租约已过期**的任务——重置活着的租约会让两个进程跑同一条指令。
...
路由是**纯词法**的（[08](../layers/42-Agents.md) §3.4）——只有命中声明的关键词或示例才进管线，其余一律走普通 ReAct。三条链的触发词：
...
`tdd_evidence` 和 `review_verification` 共享同一个精神：**只有拿到成功证据才能报告完成**。这条约束在人格文件、管线指令、门禁三处各写了一遍（[08](../layers/42-Agents.md) §11.1 ④），因为"没做却说做了"是 Agent 最严重的失效模式。
...
**① 第一次 `shell` 调用弹审批。** `shell` 是唯一 `approval_policy=always` 的工具（[08](../layers/42-Agents.md) §4.1）——每次都要点。这不是可以关掉的配置，因为它能执行任意命令，风险不可从参数推断。

**② 读文件不弹审批，但读 `.env` 会。** 十一个只读工具默认 `never`，但敏感文件名会把风险升到 high（[06](../subsystems/23-工具与安全.md) §11.1.2）。`.env.example` 这类模板明确放行。

**③ 第一次改文件前会被"挑战"。** 事实门要求先调查再动手（[06](../subsystems/23-工具与安全.md) §7）。它是**可重试的**——补上调查就能继续，不是拒绝。

**④ 上下文快满时会自动压缩。** 你会看到"正在压缩"的提示，这是 `CONTEXT_COMPRESSION_START/END` 事件（[03](../architecture/10-系统架构.md) §3）。也可以手动 `/compact`。

**⑤ 记忆不会立刻出现。** 记忆编译按阈值触发，且要过三道守卫加一道 LLM 评审（[05](../subsystems/21-记忆系统.md) §7）。一次对话不一定产生记忆——这是刻意的，宁可少记不可记错。

**⑥ 跨会话记得的是"结论"不是"对话"。** 两份渲染视图各有字符上限（context 4 000 / durable 10 000），整份注入 prompt，**没有检索**（[05](../subsystems/21-记忆系统.md) §3）。所以它记的是提炼过的事实，不是原始对话。

**⑦ 后端是 shell 自己起的。** 关掉 shell 会一并回收后端（[11](../layers/44-Shell.md) §10.2.1 讲了四道防孤儿措施）。如果端口被占，检查有没有残留进程。

**⑧ 改了代码要重启后端。** uvicorn 只 import 一次模块。`/api/health` 的 `stale` 字段会告诉你当前进程是不是在跑旧代码（[09](../layers/43-Server.md) §10.1）。

**⑨ 中转站的模型列表不可信。** `/v1/models` 列出来不等于能用（[07](../subsystems/25-LLM集成.md) §15）。换模型前先直接 `curl` 打一次。

**⑩ 崩溃的 run 可以续，但有条件。** 七道校验里最常拦人的是"会话已有更新的用户轮次"（[09](../layers/43-Server.md) §4.7）——崩了之后你又说了新的话，那个 run 就续不了了。
...
**③ 明明有 24 个技能目录，只认 16 个。** 判定标准是有没有顶层 `SKILL.md`，其余 8 个只是参考文档（[08](../layers/42-Agents.md) §1.1）。
...
| 自定义 hook | `~/.agent-smith/hooks.yaml` | 全局，在内建 hook 之后加载（[04](../subsystems/20-Agent-Loop.md) §8.5.5） |
...
**② 确认 prompt 缓存生效。** 稳定前缀是第 1–10 层（[04](../subsystems/20-Agent-Loop.md) §2.4），provider 按字节前缀匹配。如果你在前十层里塞了会变的内容（比如把时间戳写进 `SMITH.md`），缓存就永远命不中。改完 `SMITH.md` 后观察 `/token` 的缓存读取比例能确认。

**③ 别把长内容写进人格文件。** 四份人格文件全在稳定前缀里，**每一次请求都会完整发送**（缓存命中时便宜，但仍占上下文预算）。真正长的内容应该做成技能（按需加载）或者放进项目 `SMITH.md`（只在该项目生效）。

顺带一提，`/token` 面板的数据来自 summary 投影而非 trace（[10](../subsystems/27-可观测性.md) §2），所以它读起来很快，可以随时看。它区分了 `local-estimate` 和 provider 上报的真实用量——前者是没有用量数据时的兜底估算，不要拿它当账单核对。
...
**③ 安全提示不是障碍，是可以调的。** 每次 `shell` 都要批准、第一次改文件要先调查——这些都有明确的机制和边界（[06](../subsystems/23-工具与安全.md)）。理解它们之后，你会知道哪些可以配置放宽（会话白名单）、哪些不能（敏感文件），以及为什么。**被安全措施烦到的时候去读它的设计，通常比直接关掉它划算。**
...
| 目的 | 读 |
|---|---|
| 想知道刚才那条命令背后跑了什么 | [03 · 架构总览](../architecture/10-系统架构.md) |
| 想改 prompt 里的某一层 | [04 · Engine 核心执行](../subsystems/20-Agent-Loop.md) |
| 想加一个工具 | [08 · Agents 内容层](../layers/42-Agents.md) |
| 想调模型配置 | [07 · LLM 集成](../subsystems/25-LLM集成.md) |
| 想看运行档案 | [10 · 可观测性与诊断](../subsystems/27-可观测性.md)，或直接 `/runs`、`/trace` |
