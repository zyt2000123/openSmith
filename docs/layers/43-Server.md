...
> **定位**：`server/` 6.4k 行——34 个路由端点 + `/api/health`、13 个 service、并发控制、自动任务调度器、本地鉴权。
...
**路由极薄**：`agent.py` 的 31 个端点只占 278 行（另有 `config.py` 3 个端点 105 行），平均每个端点 9 行——取参、调 service、返回。所有逻辑在 service 层。
...
`agent.py` 的 31 个端点里有 20 多个都注入 `AgentService`。它把多个专门 service 聚合成一个，让路由层只依赖一个东西。
...
**五个集合里有四个直接来自 `engine/llm/config_fields.py`**，只有超时字段是本地的。这就是那份"声明一次、四处派生"的设计在服务端的落点（见 [07 · LLM 集成](../subsystems/25-LLM集成.md) §2.2）。
...
实测提醒：**中转站列出的模型不等于能用**（见 [07 · LLM 集成](../subsystems/25-LLM集成.md) §12）。这个端点只做发现，不做可用性验证。
...
关闭顺序的依赖链（见 [03 · 架构总览](../architecture/10-系统架构.md) §8.1）：**运行中的任务 → 它们用的客户端 → 审计链头 → 数据库连接**。

启动时的 `_reconcile_startup_observability()` 处理"崩在写 trace 之后、写 summary 之前"这个窗口——这是 [03 · 架构总览](../architecture/10-系统架构.md) §9.2 讲过的。
...
审计链的封存（`seal()`）要记下"此刻的链头是谁"。这个断言只有在**没有任何 run 还在往日志里追加**的时候才成立——所以必须排在 ④ 之后。如果提前封存，之后又有 run 追加了几条，下次校验会看到链长过锚点，报成回滚（见 [13 · Common 基础设施](40-Common.md) §6.10 的误报问题）。
...
注意两个 `except` 的宽度不同：恢复用 `(RunStateError, OSError)`（预期内的失败），统计用 `Exception`（更宽）。这和 [10 · 可观测性与诊断](../subsystems/27-可观测性.md) §2.2 的纪律一致——越是"纯观测"的东西，捕获得越宽，因为它绝不该影响主流程。
...
**① 路由极薄。** `agent.py` 的 31 个端点只占 278 行。所有逻辑在 service，所以换一个前端协议（比如 WebSocket）不需要重写业务。
...
这五条里，①②在 [12 · MCP 集成](../subsystems/26-MCP集成.md) 的连接池同样成立（先建后拆、失败不缓存），④⑤是这一层特有的——因为只有它同时面对外部请求和长时后台任务。
...
| 文件 | 数量 | 覆盖 |
|---|---|---|
| `test_common_infrastructure.py` | 38 | 见 [13 · Common 基础设施](40-Common.md) §10 |
...
| 想深入 | 读 |
|---|---|
| 可观测的 7 个端点返回什么 | [10 · 可观测性与诊断](../subsystems/27-可观测性.md) |
| 引擎怎么被装配 | [03 · 架构总览](../architecture/10-系统架构.md) §2 |
| Shell 怎么消费这些端点 | [11 · Shell 终端 UI](44-Shell.md) |
| 配置校验的字段派生 | [07 · LLM 集成](../subsystems/25-LLM集成.md) §2.2 |
| cron 语法与租约 | [02 · 快速上手](../guide/02-快速上手.md) §13 |
