...
这是管线场景独有的（见 [03 · 架构总览](../architecture/10-系统架构.md) §3.2）：
...
如果改成"每个渲染组件自己净化"，加一个新面板的人就有可能忘记。放在解码边界，数据进入前端时就已经是干净的，之后无论经过多少渲染路径都安全。这和 [13 · Common 基础设施](40-Common.md) §9.1 ② 的"把约束放在收敛点"是同一条思路。
...
这条和 [09 · Server API 层](43-Server.md) §4.8 是同一个原则在两侧的体现——**显示的和存储的必须一致**，任何一侧单方面清理都会制造幻觉。
...
因为 `api_key` 是**只写不读**的（见 [07 · LLM 集成](../subsystems/25-LLM集成.md) §2.2），配置读回来时这个字段永远是空的。于是向导要区分三种状态：
...
**连续 3 次失败才算"停滞"**（`MEMORY_FAILURE_STREAK_THRESHOLD`）——一次瞬时失败不该在状态栏亮红灯。这和记忆系统本身"`rejected` / `failed` 不计入跳过计数"（见 [05 · 记忆系统](../subsystems/21-记忆系统.md) §9.1）是同一条判断：**区分"这次没成"和"一直不成"**。
...
`dev-server.ts`（473 行）的完整决策流见 [02 · 快速上手](../guide/02-快速上手.md) §6.1。这里补三个实现要点。
...
这五条里，①②在 [09 · Server API 层](43-Server.md) 也成立（归属校验放仓库层、显示与存储一致），③④⑤是终端 UI 特有的——因为只有它同时受限于屏幕尺寸和人的反应时间。
...
**Shell 的复杂度不在功能，在于终端的约束。**

没有 DOM、没有 CSS、宽度要自己算、重绘代价高、还要防转义序列注入。10.7k 行实现里，注入防护只有 87 行却是安全价值最高的部分，而 `bridge.ts`、`api.ts`、`index.tsx` 三个文件就占了三分之一。

它承担的独特职责是**把用户的决定和系统的状态对齐**——审批提示里显示的命令必须就是要执行的那条，终态文案必须区分"失败"和"在等你"，中断时缓冲里的文本不能再出现。这些在后端只是数据正确性问题，在这里是**用户会据此做不可逆决定**的问题。
...
| 想深入 | 读 |
|---|---|
| 27 种事件的完整语义 | [03 · 架构总览](../architecture/10-系统架构.md) §3 |
| 启动协商的完整流程 | [02 · 快速上手](../guide/02-快速上手.md) §6 |
| 20 个斜杠命令各做什么 | [02 · 快速上手](../guide/02-快速上手.md) §7 |
| Shell 打的端点 | [09 · Server API 层](43-Server.md) §2 |
| 审批面板显示的内容从哪来 | [06 · 安全与安全边界](../subsystems/23-工具与安全.md) §6.3 |

---

> **一句话收尾**：终端 UI 的 bug 大多读代码发现不了——Ink 的 `Box` 只换行不裁剪、`<Static>` 在特定条件下整卷重印、宽字符的列宽计算，这些只在真实终端里以特定宽度运行才暴露。单元测试能覆盖状态转换，覆盖不了"渲染到 80 列上是什么样"。**改任何涉及布局或宽度的代码，真跑一遍比多写三个断言有用。**
