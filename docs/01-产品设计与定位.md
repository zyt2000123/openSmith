...
> **当前实现说明**：Smith 是一个 local-first、terminal-native 的单 Agent 工作台。本文描述产品边界，不把规划中的多 Agent、团队协作或桌面应用写成现有能力。
...
| 可替换模型 | OpenAI-compatible、Anthropic 与 Gemini 协议接入，可按 interactive/gate/background 路由 | `engine/llm/` |
...
- `coding`：功能开发、修复与重构身份，路由到 `coding` pipeline，依次执行理解、规划、可选架构、实施、验证并接受 gate 约束。

这条设计选择的目的，是先把单条运行链路的权限、恢复、事件和交付语义做完整，再评估是否值得引入子 Agent 调度成本。

## 核心使用场景

1. **可验证的软件变更**：用户要求修 bug、重构或实现功能。系统通过 coding pipeline 组织阶段产出，并在需要工具副作用时请求审批。
...
| 范围 | 当前选择 |
| --- | --- |
| 客户端 | Ink/React Shell；没有 SwiftUI/macOS 原生客户端。 |
| 协作 | 单用户、本地 server；没有 team API、圆桌会议或子 Agent 执行。 |
