...
>
> 实现注（2026-07-29）：本文的 P0 已采用为 Engine-owned 的 `memory/nudge.py`，每 20 个有效记忆回合受限地产生候选；为保持 `memory_ops` 对主 Agent 隐藏，候选不会通过该工具调用。当前规范以 [`21-记忆系统.md`](../21-记忆系统.md) 与 [`MEMORY_POLICY.md`](../../engine/memory/MEMORY_POLICY.md) 为准。
...
> 一并移除了 episode 层与检索索引。当前实现见 [21 · 记忆系统](../21-记忆系统.md)。
