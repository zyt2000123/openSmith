...
> **已归档 —— 不是当前事实。**
> 本文已被 [20 · Agent Loop](../subsystems/20-Agent-Loop.md) 取代；两者冲突时以那一篇和源码为准。
> 裁决依据：探针 6:8 落后，缺 27 种 EventType 与三条执行路径。
...
这就是 [02 · 快速上手](../guide/02-快速上手.md) §10.1 讲的三级收窄的第三级。
...
即：只有 `TOOL_CALL_RESULT` 且**没出错**才算证据。这条判断直接决定了记忆系统里那一轮证据的 `kind`，进而决定它能不能建立 `Verified Outcomes` 条目（见 [05 · 记忆系统](wiki-05-记忆系统.md) 的放置守卫）。
...
| `INCOMPLETE` / `FAILED` / `CANCELLED` **可以回到 `RUNNING`** | 这就是续跑的实现。对应 [09 · Server API 层](../layers/43-Server.md) §4.7 第 3 条校验 |
...
`mismatched_fields()` 返回的是**列表而不是布尔**——错误消息里能说清到底哪几个字段对不上（agent_id？session_id？identity_id？）。这个设计和 [13 · Common 基础设施](../layers/40-Common.md) §6.6 里 `verify_chain` 返回具体失败原因是同一种考虑：**校验失败时，"哪里不对"比"不对"有用得多**。
...
第三种是这里的选择。它和 [10 · 可观测性与诊断](../subsystems/27-可观测性.md) §10.1 分支①"隔离损坏的 trace"是同一条原则：**坏数据要被隔离，不能被清理掉**——清理等于销毁现场。
...
它们被排除在同步写之外。这和 [13 · Common 基础设施](../layers/40-Common.md) §6.11 里 `sync=False` 的延迟同步是配套的取舍：**高频、低价值的记录走宽松路径，状态转移走严格路径**。
...
状态文件里的文本字段（reason、错误详情）**限长 200 字符**。理由和 [11 · Shell 终端 UI](../layers/44-Shell.md) §5.8.3 的 2 000 字符上限一样：错误详情可能包含模型的长输出或一整个异常栈，不限长会让一个状态文件涨到几 MB，而它每次状态转移都要重写一遍。
...
这条和 [10 · 可观测性与诊断](../subsystems/27-可观测性.md) §10.5 的"可观测性失败不能让运行失败"是同一个方向，但对象不同：这里是**运行控制状态**。
...
这里用的是 `issubclass` 而非 `Protocol`——和 [07 · LLM 集成](../subsystems/25-LLM集成.md) §2.4 的选择相反。原因是场景不同：hook 是**用户提供的扩展**，明确继承一个基类能让用户从 IDE 得到方法签名提示，也让"你漏实现了某个方法"在加载时就报错；而 LLM 客户端的包装器需要的是"不继承也能顶替"的灵活性。

### 8.5.4 每一步失败都只跳过这一个 hook

三个加载方法里，任何一步失败（缺 `module`、缺 `class`、类加载不出来、基类不对）都是 `logger.error` + `return None`，**不抛异常**。

这意味着：一个写错的 hook 配置不会让整个引擎起不来，其余 hook 照常加载。这和 [12 · MCP 集成](../subsystems/26-MCP集成.md) §9 的"一个坏 server 不影响其余"是同一条隔离原则。
...
需要注意的是它**不能覆盖非 hook 的安全边界**：`tool_guard.py` 是不可绕过的硬守卫，不在 hook 体系里（见 [06 · 安全与安全边界](wiki-06-安全与安全边界.md)）。hook 能做的只是在守卫**之外**再加一层拦截，不能拆掉守卫本身。
...
**Engine 知道怎么执行一个任务，但不知道谁在调它。**

16 层 prompt、60 次迭代上限、7 态运行状态机、三条管线——所有执行机制都在这里，而所有"外部事实"（工作目录、会话历史、身份、LLM 客户端）都是参数传进来的。这条约束让引擎能被嵌进任何宿主，也让 1 041 个测试不需要起 HTTP 服务就能跑。
...
| 想深入 | 读 |
|---|---|
| `ToolGuard` 那 1365 行到底在拦什么 | [06 · 安全与安全边界](wiki-06-安全与安全边界.md) |
| 记忆视图怎么被编译出来 | [05 · 记忆系统](wiki-05-记忆系统.md) |
| provider 适配器怎么产出这些事件 | [07 · LLM 集成](../subsystems/25-LLM集成.md) |
| 三条管线的门禁具体检查什么 | [08 · Agents 内容层](wiki-08-Agents-内容层.md) |
