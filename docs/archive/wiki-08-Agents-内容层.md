...
> **已归档 —— 不是当前事实。**
> 本文已被 [42 · Agents 内容层](../layers/42-Agents.md) 取代；两者冲突时以那一篇和源码为准。
> 裁决依据：探针 5:7 落后，缺 subagents 目录与四层收敛链，且仍写“19 个工具”。
...
它的 `permission_level="write"`、`approval_policy="policy"` 都定义好了，但 `agents/smith/config.yaml` 的 `tools.enabled` 里没有它——**能力存在但被静态关闭**，因为记忆写入必须走编译管线的证据裁决（见 [05 · 记忆系统](wiki-05-记忆系统.md)）。
...
**③ `shell` 是唯一 `always` 的。** 其余写工具用 `policy`（按配置的风险等级决定），只有 `shell` **每次都要用户批准**——因为它能执行任意命令，风险不可从参数推断。这和 [12 · MCP 集成](../subsystems/26-MCP集成.md) §6.3 里远程 MCP 工具一律 `always` 是同一个判断：**能力边界不可知时，一律要人点头**。
...
IP 检查和 [07 · LLM 集成](../subsystems/25-LLM集成.md) §2.2.1 的 `validate_llm_base_url` 是同一套 SSRF 防护，只是那里保护的是凭据不外泄，这里保护的是**不要拿 Agent 当内网跳板**。
...
抓回来的内容用围栏包起来交给模型，明确标注"这是外部不可信内容"。这和 [04 · Engine 核心执行](wiki-04-Engine-核心执行.md) §2.2 里 `learned_context` 和 `durable_context` 的围栏是同一个手法——**任何非用户授权的文本进入 prompt 时都要标明来源**，否则一个网页里写着"忽略之前的指令"就可能生效。
...
这和 [12 · MCP 集成](../subsystems/26-MCP集成.md) §3.1 的 MCP 子进程环境白名单是完全相同的思路——项目里凡是"要跑一个可能不受控的子进程"的地方，都用同一套 credential-free 环境。
...
`_redact_url_credentials()` 则处理输出侧：git 的错误消息里可能包含 `https://user:token@host/repo` 形式的远端地址，脱敏后才能进入会话记录和日志。这和 [12 · MCP 集成](../subsystems/26-MCP集成.md) §9.5 的 `_redact_url` 是同一类处理。
...
`LLMGate` 是"便宜的启发式预过滤 + LLM 语义验证"（见 [04 · Engine 核心执行](wiki-04-Engine-核心执行.md) §6.3）。
...
这和 `eval_guard`（[06 · 安全与安全边界](wiki-06-安全与安全边界.md) §8）是同一类防御：**防止 Agent 修改评判标准而不是满足评判标准**。
...
三条管线的完整声明见 [02 · 快速上手](../guide/02-快速上手.md) §14。这里补两个 YAML 字段的语义：
...
| `test_tool_guard.py` | 57 | 硬守卫（见 [06 · 安全与安全边界](wiki-06-安全与安全边界.md)） |
...
最后一个是假阳性防护，和 [05 · 记忆系统](wiki-05-记忆系统.md) §12.2 的"冠词必需"是同一类考虑：`.env.example` / `.env.template` 是**要提交进仓库、给人看的文档**，把它们也拦下来会让 Agent 连项目的配置说明都读不了。
...
两个 `symlinked_git_dir` 测试针对的是一种规避手法：把 `.git` 做成指向别处的符号链接，让基于路径前缀的检查失效。这和 [13 · Common 基础设施](../layers/40-Common.md) §2.6 的逐段符号链接检查是同一类防护。
...
| 想深入 | 读 |
|---|---|
| 工具怎么被注册和执行 | [04 · Engine 核心执行](wiki-04-Engine-核心执行.md) §5 |
| 门禁的三态与两层结构 | [04 · Engine 核心执行](wiki-04-Engine-核心执行.md) §6 |
| 工具的安全元数据怎么被消费 | [06 · 安全与安全边界](wiki-06-安全与安全边界.md) §3 |
| 人格文件怎么进 prompt | [04 · Engine 核心执行](wiki-04-Engine-核心执行.md) §2 |
| 三条管线各走一遍 | [02 · 快速上手](../guide/02-快速上手.md) §14 |
