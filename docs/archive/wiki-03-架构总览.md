...
> **已归档 —— 不是当前事实。**
> 本文已被 [10 · 系统架构](../architecture/10-系统架构.md) 取代；两者冲突时以那一篇和源码为准。
> 裁决依据：探针 3:6 落后，缺依赖方向、子 Agent 委派、记忆无检索层三条。
...
| 子域 | 行数 | 对应文档 |
|---|---|---|
| `execution/` | 7 635 | [04 · Engine 核心执行](wiki-04-Engine-核心执行.md) |
| `memory/` | 4 014 | [05 · 记忆系统](wiki-05-记忆系统.md) |
| `llm/` | 3 182 | [07 · LLM 集成](../subsystems/25-LLM集成.md) |
| `safety/` | 2 749 | [06 · 安全与安全边界](wiki-06-安全与安全边界.md) |
| `observability/` | 2 100 | [10 · 可观测性与诊断](../subsystems/27-可观测性.md) |
| `context/` | 1 871 | [04 · Engine 核心执行](wiki-04-Engine-核心执行.md) §2 |
| `tool/` | 1 527 | [04 · Engine 核心执行](wiki-04-Engine-核心执行.md) §5 |
| `mcp/` | 1 212 | [12 · MCP 集成](../subsystems/26-MCP集成.md) |
| `skill/` | 829 | [04 · Engine 核心执行](wiki-04-Engine-核心执行.md) §8 |
| `sandbox/` | 789 | [06 · 安全与安全边界](wiki-06-安全与安全边界.md) §9 |
| `identity/` | 378 | [08 · Agents 内容层](wiki-08-Agents-内容层.md) §3 |
...
代价是**每一条 START 路径都必须保证有对应的 END**，包括异常路径。[11 · Shell 终端 UI](../layers/44-Shell.md) §7.0 ⑥ 那条"结果没到的工具要清理"就是这个契约被打破时的兜底：一个只有 START 没有 RESULT 的工具会让 HUD 的转圈图标永远不停。
...
`INCOMPLETE` 与 `FAILED` 的区别在**可恢复性**：前者进 [04 · Engine 核心执行](wiki-04-Engine-核心执行.md) §7.1.1 那张状态转移表里"可以回到 RUNNING"的一档。这个区分一路传到终端文案（[11 · Shell 终端 UI](../layers/44-Shell.md) §5.8）：`incomplete + awaiting_user_input` 显示成"等你回复"而不是"失败了"。
...
2. 走 SSE 传给终端
3. 被 `_execution_events()` 从旧文件读回来重建（[10 · 可观测性与诊断](../subsystems/27-可观测性.md) §10.4）

三个用途都跨版本：**今天的引擎要能读懂上个月写下的 trace**。所以事件类型的字符串值一旦发布就不能改——改了等于让所有历史 trace 变成无法解析的数据。

这也是为什么读侧到处是 `except ValueError: continue`（[10 · 可观测性与诊断](../subsystems/27-可观测性.md) §10.4）和 `.get(key, default)`：**新版本要容忍旧数据里没有的字段，旧版本要容忍新版本加的类型**。枚举只增不改，加上读侧宽容，两者共同保证了向前向后兼容。
...
| 前提 | 谁负责 | 少了会怎样 |
|---|---|---|
| run 状态可持久化且状态转移合法 | `run_state.py`（[04](wiki-04-Engine-核心执行.md) §7.1） | 不知道这个 run 崩在哪一步、能不能续 |
| trace 完整且可验证 | `trace_store.py` + 哈希链（[13](../layers/40-Common.md) §6） | 无法重建事件序列，也无法判断记录是否被改过 |
| summary 与 trace 可对账 | `runtime.py` 的四条分支（[10](../subsystems/27-可观测性.md) §10.1） | 重复累加 token，或者 run 在列表里消失 |
| 用户消息绑定到 run | `message_id` 字段（[09](../layers/43-Server.md) §4.7 第 5 条） | 不知道该重放哪一轮对话 |
...
**① 校验全部在改状态之前。** 七道校验（[09](../layers/43-Server.md) §4.7）跑完才开 SSE 流。一旦流开了就只能在流里报错，客户端已经在读了。

**② 旧输出直到新输出产生才丢弃。** 续跑失败时原来那半截回复还在（[09](../layers/43-Server.md) §4.8）。这让续跑成为"要么改进要么不变"的操作。

**③ 锚点要先清后写。** `reopen → append → seal` 三步（[04](wiki-04-Engine-核心执行.md) §7.1.2）。不先清陈旧锚点，合法的续写会被哈希链校验报成篡改——这是"防篡改系统必须能区分改动与增长"的又一个实例（[13](../layers/40-Common.md) §6.10）。
...
| 子系统 | 测试数 | 对应文档 |
|---|---|---|
| 工具与守卫 | 196 + 156 | [08](wiki-08-Agents-内容层.md) §10.1、[06](wiki-06-安全与安全边界.md) §11.1 |
| 记忆 | 157 | [05 · 记忆系统](wiki-05-记忆系统.md) §14.1 |
| LLM | 97 | [07 · LLM 集成](../subsystems/25-LLM集成.md) §18 |
| MCP | 51 | [12 · MCP 集成](../subsystems/26-MCP集成.md) §17 |
| 可观测性 | 47 | [10 · 可观测性与诊断](../subsystems/27-可观测性.md) §15 |
| Common 基础设施 | 38 + 20 | [13 · Common 基础设施](../layers/40-Common.md) §10 |
| Shell | 304 | [11 · Shell 终端 UI](../layers/44-Shell.md) §13.0 |
| Server 各 service | 220 | [09 · Server API 层](../layers/43-Server.md) §13 |

### 10.2 测试分布揭示的三件事

**① 安全与工具占了最大比重（352 个）。** 这不是因为它们代码最多（`safety/` 2 749 行只占 engine 的 10%），而是因为**每一条安全规则都要在多个入口各测一遍**（[06 · 安全与安全边界](wiki-06-安全与安全边界.md) §12.2）。测试数与代码量的比例，反映的是"这段代码错了有多严重"。

**② 记忆的 157 个里有 32 个只测三道守卫。** 守卫代码只有 283 行，却占了记忆测试的五分之一——因为它是唯一"判错会污染长期记忆"的地方，而记忆会被注入未来每一次 prompt（[05 · 记忆系统](wiki-05-记忆系统.md) §15.1）。

**③ Shell 的 304 个是第二多。** 终端 UI 通常被认为"测不了"，这里的做法是把可测的部分（状态转换、SSE 解码、输入路由、注入防护）和不可测的部分（实际渲染效果）分开。304 个测试覆盖前者，后者靠真实终端手动验证（[11 · Shell 终端 UI](../layers/44-Shell.md) §14.2 末尾）。
...
| 约束 | 三处实现 | 漂移记录 |
|---|---|---|
| 凭据配置文件名 | `_CREDENTIAL_CONFIG_NAMES` / `_CREDENTIAL_CONFIGS` / `SECRET_EXCLUDED` | [06](wiki-06-安全与安全边界.md) §3.4（`.npmrc`） |
| SSH 私钥文件名 | `_SENSITIVE_KEY_NAME_RE` / `_PRIVATE_KEY_NAME_RE` | [06](wiki-06-安全与安全边界.md) §9.2（`id_rsa_old`） |
...
| 加一个路径工具函数 | `common/paths.py` **仅当**三层都要用 | 只有一个调用方就放调用方那里（[13](../layers/40-Common.md) §9.1 末尾） |

三条容易搞反的：

**① "改工具能做什么" vs "改工具允许做什么"。** 前者在 `agents/`，后者在 `engine/safety/`。一个工具可以声明自己需要 `write` 权限，但**它不能声明自己不需要审批**——`approval_policy` 虽然写在 `TOOL_META` 里，最终仍要过守卫这一关（[06](wiki-06-安全与安全边界.md) §1）。内容层的声明是**输入**，不是**结论**。
...
终端 UI 的复杂度不在功能而在**约束**：没有 DOM、没有 CSS、宽度要自己算、重绘代价高、还要防终端转义注入（[11](../layers/44-Shell.md) §5.9）。这 10.7k 行里，`index.tsx` 1 299 行 + `bridge.ts` 1 103 行 + `api.ts` 1 053 行就占了三分之一，剩下的是十几个渲染组件。相比之下一个同等功能的 Web 前端不会更小。
...
**`common/`：不能有业务逻辑。** 代价是有些明显可以复用的东西不能放进去（只有一个调用方的函数、带领域含义的常量）。收益是三层都能安全依赖它——一旦它开始装业务逻辑，依赖方向就会开始反转（[13](../layers/40-Common.md) §9.1 末尾）。
...
**`server/`：路由必须薄。** 代价是每加一个端点都要写两处（router + service）。收益是同一套逻辑能被 HTTP、自动任务、续跑三条入口复用（[09](../layers/43-Server.md) §12.1 末尾）。

**`shell/`：所有后端通信走 `bridge.ts`。** 代价是加一个 API 调用要在 bridge 里加方法而不能在组件里直接 fetch。收益是竞态防护、状态更新、批处理都能集中实现（[11](../layers/44-Shell.md) §5.5）。
...
**① 引擎要能脱离这个 server 存在。** 这不是假设——`RecordingLLM` / `ReplayLLM`（[07](../subsystems/25-LLM集成.md) §12）、`engine/tests/` 里的 1 041 个测试、以及 `RuntimeContext.default_working_dir` 那条"引擎绝不从自己进程的 cwd 推导工作区"（§2.2），三者都建立在"引擎不知道谁在调它"这个前提上。如果引擎里有一行 `from fastapi import ...`，这 1 041 个测试就都要起一个 HTTP 服务才能跑。
...
| 想深入 | 读 |
|---|---|
| ReAct 循环、管线、门禁、工具执行的细节 | [04 · Engine 核心执行](wiki-04-Engine-核心执行.md) |
| 16 层 prompt 的每一层是什么 | [04 · Engine 核心执行](wiki-04-Engine-核心执行.md) |
| 事件怎么变成运行档案 | [10 · 可观测性与诊断](../subsystems/27-可观测性.md) |
| 端点清单与 service 分工 | [09 · Server API 层](../layers/43-Server.md) |
| Shell 怎么消费这个事件流 | [11 · Shell 终端 UI](../layers/44-Shell.md) |
...
最后提醒一点：本篇给出的所有行数都附了统计口径（§1.3、§1.4）。跨篇比对数字时先确认口径是否一致——`shell` 在 [01 · 总览](wiki-01-总览.md) 里记的是 16.4k（含测试），本篇记的是 10.7k（不含测试），两个都对。口径不同不是矛盾，但把它们直接相比就会得出错误结论。
