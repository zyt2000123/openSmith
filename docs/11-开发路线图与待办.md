...
| 层 | 完成项 |
|---|---|
| common/ | 路径与私有运行目录、SQLite 连接（WAL/外键/忙等待）、YAML 读写与深合并、内置技能资源同步 |
...
- **涉及文件**:
  ...
  - 修改 `engine/context/assembler.py` — 记忆层改为注入编译后的 `memory.md`
...
- **涉及文件**:
  ...
  - 修改 `engine/execution/orchestration/lifecycle.py` — 统一运行流接入后台结果
...
- **涉及文件**:
  ...
  - `engine/execution/orchestration/preparation.py` — 工具注册和运行时准备
...
- [ ] **待检查**
- **涉及文件**:
  - `engine/execution/pipeline/gate.py` — 门禁实现
  - `engine/execution/pipeline/skill_chain.py` — SkillChain + backtrack_map
  - `engine/execution/pipeline/backtrack.py` — FailureLoopGuard
- **检查要点**:
...
- [x] **已修复**
- **详情**: `engine/execution/orchestration/lifecycle.py` 中的流式路径已累积完整回复文本再保存记忆
- **涉及文件**: `engine/execution/orchestration/lifecycle.py` — 运行流生命周期
...
- **涉及文件**: `engine/execution/orchestration/agent_loop.py` — Pipeline 分派
