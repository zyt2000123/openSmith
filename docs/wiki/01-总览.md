...
**写作过程中抓到过三处错误**：事件类型一度写成 26（实际 27）、engine 行数一度算成 387 818（`.cache` 没排除）、`common` 行数写成 1.1k（实际 1.4k）。三处都已修正，且都是靠交叉核对发现的。这也是为什么每个数字都附了核实方式（[14 · 术语表](./14-术语表.md) §10.4）——**能被复算的数字才值得写进文档**。
