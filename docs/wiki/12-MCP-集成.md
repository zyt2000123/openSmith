...
docstring 里 "Mirrors the credential-free `_safe_environment` used for model-requested shells" 这句是在指认同源：模型请求执行 shell 命令时走的是同一套构造逻辑（见 [06 · 安全与安全边界](./06-安全与安全边界.md)）。**两处必须保持一致**——如果只加固 shell 而忘了 MCP，攻击者换条路照样能读到凭据。
...
这条设计和 [06 · 安全与安全边界](./06-安全与安全边界.md) 的整体取向一致：**信任必须来自本地配置，不能来自网络对端**。
...
| 想深入 | 读 |
|---|---|
| MCP 工具怎么进工具注册表 | [04 · Engine 核心执行](./04-Engine-核心执行.md) §6 |
