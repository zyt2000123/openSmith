...
这个"同一份数据两个版本"的模式在别处也出现过：[10 · 可观测性与诊断](./10-可观测性与诊断.md) §3.2 的 trace 两层脱敏、[12 · MCP 集成](./12-MCP-集成.md) §6.1 的配置指纹（内存里用完整值，日志里用摘要）。共同点是**内部需要精确，外部只需要够用**。
...
深度上限尤其重要。审批提示渲染在终端页脚，空间本来就小；而且深层嵌套的递归遍历有栈溢出风险——和 [12 · MCP 集成](./12-MCP-集成.md) §10.3 捕获 `RecursionError` 防的是同一类问题。
...
- **必须有限**：否则一个无人值守的自动任务会永远挂在这里，占着并发槽（见 [09 · Server API 层](./09-Server-API层.md) §5）

超时和拒绝的语义是一样的——**工具没有执行**，没有任何副作用发生。这一点在健康度统计里也被明确处理：[10 · 可观测性与诊断](./10-可观测性与诊断.md) §8.1 把超时的审批排除在工具成功率之外，因为它根本没执行过。
...
理由是一致性：如果 UI 自己推导风险等级，它和守卫的判断可能不一致——守卫按高风险要求审批，UI 却把它渲染成普通提示。用户看到的紧急程度和系统实际的判断脱钩，这正是 [11 · Shell 终端 UI](./11-Shell-终端UI.md) §14.1 ② 那条"显示的必须等于生效的"要防的。
...
这份名单和 [08 · Agents 内容层](./08-Agents-内容层.md) §10.1.3 的 `other_git_metadata_reads_stay_ordinary` 对应——读 git 历史是调查行为，不该被当成改动。
...
一句笼统的"请先调查"对模型没有指导作用，它不知道调查什么算够。**挑战要可执行**，这和 [08 · Agents 内容层](./08-Agents-内容层.md) §11.1 ① 那条"约束要能被检验"是同一个道理。
...
而 `.git/config` 能让 git 执行任意命令（见 [08 · Agents 内容层](./08-Agents-内容层.md) §4.4），所以它必须进入硬链接保护名单，即使它不算"凭据数据"。
...
`_OPTIONAL_ENV_KEYS = ("LANG", "LC_ALL", "TERM", "TZ", "NO_COLOR")` 则是环境白名单，和 [12 · MCP 集成](./12-MCP-集成.md) §3.1 的 MCP 子进程白名单**几乎相同**（少一个 `LC_CTYPE`）。项目里凡是要跑不受控子进程的地方都用同一套思路。
