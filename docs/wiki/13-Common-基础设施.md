...
这和 [12 · MCP 集成](./12-MCP-集成.md) §8.2 的锁设计是同一条纪律——**锁内绝不跨越 I/O**。
...
`except BaseException` 覆盖 `CancelledError`：请求在设 PRAGMA 期间被取消同样要清理。这和 [12 · MCP 集成](./12-MCP-集成.md) §11.1 握手失败清理是完全一样的模式。
...
> 这是本套文档里最值得记住的一个设计现场：**"从缺失推断意图"永远是可伪造的，必须让意图被显式声明并纳入完整性保护**。同样的原则在 [05 · 记忆系统](./05-记忆系统.md) 的证据溯源守卫里也出现过。
...
`unseal()` 的 docstring 描述的场景，和 [10 · 可观测性与诊断](./10-可观测性与诊断.md) §10.2 的 `reopen()` 是同一件事的底层实现：
...
`sync` 参数默认 `False`：普通追加只 `flush` 到操作系统，不强制落盘。真正的持久化由 `seal()` 统一 fsync（见 §6.3）。这是吞吐和持久性的取舍——trace 每秒可能写几百条，逐条 fsync 会让写入变成瓶颈；而崩溃时最多丢失最后未封存的一小段，可以由 [10 · 可观测性与诊断](./10-可观测性与诊断.md) §10.1 的恢复逻辑处理。
...
这六条里，②③在 [12 · MCP 集成](./12-MCP-集成.md) 同样成立，④在 [05 · 记忆系统](./05-记忆系统.md) 的证据守卫里成立，①⑤是这一层特有的——因为只有它直接操作用户的文件系统。
