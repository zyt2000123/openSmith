...
`test_e2e_smoke.py` 只有 1 个测试，但它跑通一条完整链路：建 agent → 建会话 → 发消息 → 收 SSE → 查历史。数量上不起眼，作用是**捕获集成层面的断裂**——各单元测试都过但拼起来不工作的情况。这类断裂通常出在契约的两侧各自演进：service 改了返回结构、路由还按旧字段取值，两边的单元测试都用各自的假数据，谁也发现不了。
